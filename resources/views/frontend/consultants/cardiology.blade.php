...
@section('content')
    ...
    <section id="doctorsection">
        <div class="conatiner-fluid mb-2">
            ...
            {{-- allDoctors  --}}
            @foreach ($cardiology_doctor as $doctorspecialist)
                {{-- Cardiology   --}}
                @if ($doctorspecialist->specialist == 'Cardiology')
                    @foreach ($cardiology_doctor as $doctor)
                            ...
                            @foreach ($doctor->availabilities as $doctoravailabilty)
                                ...
                                {{-- -{{ $doctoravailabilty->from }} - 
                                    ...
                                    @if ($doctor->availabilities->contains('status', 'Inactive'))
                                        <a href="#" class="mt-2 btn btn-outline-primary disabled"
                                            aria-disabled="true">Book Appointment</a>
                                        <p class="text-danger mt-2">Doctor currently inactive for appointments.</p>
                                    @else
                                        <a href="{{ route('appoinment.create', $doctor->id) }}"
                                            class="mt-2 btn btn-outline-primary">Book Appointment</a>
                                    @endif
                                </div>
                            </div>
                        </div>
